...
fun DataStatusSnackBar(dataStatus: DataStatus?, lastUpdate: String?) {
    val message: String? = when (dataStatus) {
        ...
        is DataStatus.ServiceError -> dataStatus.message?.let {
            stringResource(id = R.string.snack_bar_service_error, dataStatus.message)
        } ?: stringResource(id = R.string.snack_bar_generic_service_error)
        else -> null
    }
    if (message != null) {
        Row(
